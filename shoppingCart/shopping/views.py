...
def thank_you(request):
    ...
    if user.is_anonymous:
        ...
        items = cart_items.count() if cart_items else 0
    items_sum = calculate_sum(cart_items)
    return render(request, 'thank_you.html', {'user':user, 'items':items, 'page':'thank_you', 'cart_items':cart_items, 'sum': items_sum})
...
def confirm_order(request):
    ...
    cart_items = Item.objects.filter(cart__user=user, cart__checked_out=False)
    items_sum = calculate_sum(cart_items)
    cartt = Cart.objects.get(checked_out=False, user=user)
    cartt.checked_out = True
    cartt.save()
    User.objects.filter(username=user.username).delete()
    return render(request, 'thank_you.html', {'user':user, 'items':0, 'page':'cart', 'cart_items':cart_items, 'sum': items_sum, 'shopping':'Continue Shopping'})
